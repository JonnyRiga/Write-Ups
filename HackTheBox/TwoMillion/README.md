...
The response is ROT13-encoded. Decoding via CyberChef yields: *"In order to generate the invite code, make a POST request to `/api/v1/invite/generate`"*.
...
> **Note:** The invite code is session-specific and rotates on box reset. It can also be generated by calling `makeInviteCode()` directly in the browser console.

![Invite code generation via API](screenshots/twomillion_invite_generation.png)
...
```bash
curl -s http://2million.htb/api/v1/user/auth -H 'Cookie: PHPSESSID=<REDACTED>' | jq .
```

```json
{"loggedin": true, "username": "jhaxx", "is_admin": 0}
```

### Mass Assignment — Ruled Out

Before targeting the admin settings endpoint, we test whether `is_admin` can be injected during registration. Intercepting the `/api/v1/user/register` POST in Burp and appending `"is_admin": 1` to the JSON body has no effect — the resulting account still has `is_admin: 0`. The registration endpoint ignores unrecognised fields. The settings update endpoint is the actual attack surface.

### IDOR — Gaining Admin via Settings Update

Poking at `PUT /api/v1/admin/settings/update` step-by-step reveals required parameters through its error responses: first `Content-Type: application/json`, then `email`, then `is_admin`. Submitting all three:
...
```bash
curl -s http://2million.htb/api/v1/admin/auth -H 'Cookie: PHPSESSID=<REDACTED>' | jq .
```
...
![Admin privilege confirmed](screenshots/twomillion_admin_auth.png)
...
`www-data` appears embedded in the certificate Subject field — the `username` value is passed unsanitized to a shell command, most likely via a PHP `exec()` call of the form `generate_vpn <username>`. We escalate to a full reverse shell:
...
![Reverse shell as www-data](screenshots/twomillion_revshell.png)
...
![User flag](screenshots/twomillion_user_flag.png)
...
![Root shell via CVE-2023-0386](screenshots/twomillion_root.png)
